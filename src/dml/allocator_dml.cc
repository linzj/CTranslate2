...
namespace ctranslate2 {
  namespace dml {
    ...
    public:
      DMLAllocator(int device_index) : _device_index(device_index) {
        // No explicit initialization needed here beyond base class for now.
        // DML devices are initialized globally in backend_dml.cc via std::call_once.
      }

      ~DMLAllocator() override {
        // Allocated resources are managed by ComPtr, which automatically releases.
        // No explicit deallocation for individual resources is typically needed here.
      }

      void* allocate(size_t size, MemType mem_type = MemType::DM_BUFFER) override {
        ...
        switch (mem_type) {
          case MemType::DM_UPLOAD:
            heap_props.Type = D3D12_HEAP_TYPE_UPLOAD;
            break;
          case MemType::DM_READBACK:
            heap_props.Type = D3D12_HEAP_TYPE_READBACK;
            break;
          case MemType::DM_BUFFER:
          default: // DM_BUFFER as default
        ...
        DML_CHECK(get_d3d12_device()->CreateCommittedResource(
          ...
          &desc,
          D3D12_RESOURCE_STATE_COMMON, // Initial state, will be transitioned as needed
          nullptr,
          IID_PPV_ARGS(&dml_buffer)
        ));
      ...
      void deallocate(void* ptr, MemType mem_type = MemType::DM_BUFFER) override {
        (void)mem_type; // mem_type is not strictly needed for deallocation of D3D12 resources
        if (ptr) {
          static_cast<ID3D12Resource*>(ptr)->Release();
        }
      }

      // No other allocator methods needed for this basic implementation yet.
  ...
  Allocator& get_allocator<Device::DirectML>() {
    static dml::DMLAllocator allocator(0); // Using device_index 0, as DML often manages a single adapter by default
    return allocator;
  }
