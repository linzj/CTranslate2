...
class D3d12Module : public Module {
 public:
...
#else
  D3d12Module(bool disableAgilitySDK,
              const char* moduleName = c_direct3dModuleName);
#endif

#ifndef _GAMING_XBOX
  inline HRESULT CreateDevice(IUnknown* adapter,
                              D3D_FEATURE_LEVEL minimumFeatureLevel,
                              REFIID riid,
                              void** device) {
    return InvokeSymbol(m_d3d12CreateDevice, adapter, minimumFeatureLevel, riid,
                        device);
  }
#endif
  ...
  inline HRESULT SerializeVersionedRootSignature(
      ...
      ID3DBlob** blob,
      ID3DBlob** errorBlob) {
    return InvokeSymbol(m_d3d12SerializeVersionedRootSignature, rootSignature,
                        blob, errorBlob);
  }
 ...
 private:
  ...
  decltype(&D3D12SerializeVersionedRootSignature)
      m_d3d12SerializeVersionedRootSignature = nullptr;
...
class DmlModule : public Module {
 public:
  ...
  inline HRESULT CreateDevice1(ID3D12Device* d3d12Device,
                               DML_CREATE_DEVICE_FLAGS flags,
                               DML_FEATURE_LEVEL minimumFeatureLevel,
                               REFIID riid,
                               void** device) {
    return InvokeSymbol(m_dmlCreateDevice1, d3d12Device, flags,
                        minimumFeatureLevel, riid, device);
  }
