...
class DescriptorHeap {
 public:
  ...
  GpuEvent GetLastCompletionEvent() const { return m_completionEvent; }
...
class DescriptorPool {
 public:
  ...
  // DescriptorPool class. The caller must supply a GpuEvent that informs the
  // pool when the reserved descriptors are no longer required.
 ...
 private:
  ...
  void CreateHeap(uint32_t numDescriptors,
                  D3D12_DESCRIPTOR_HEAP_FLAGS heapFlags);
};
}  // namespace dml
}  // namespace ctranslate2