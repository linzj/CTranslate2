...
std::optional<DescriptorRange> DescriptorHeap::TryAllocDescriptors(
  ...
  if ((m_completionEvent.fence != nullptr) &&
      (m_completionEvent.IsSignaled())) {
...
DescriptorPool::DescriptorPool(ID3D12Device* device, uint32_t initialCapacity)
    : m_device(device), m_initialHeapCapacity(initialCapacity) {
...
DescriptorRange DescriptorPool::AllocDescriptors(
  ...
  for (DescriptorHeap& heap : m_heaps) {
    auto descriptorRange =
        heap.TryAllocDescriptors(numDescriptors, completionEvent, heapFlags);
  ...
  auto descriptorRange = m_heaps.back().TryAllocDescriptors(
      numDescriptors, completionEvent, heapFlags);
...
void DescriptorPool::Trim() {
  ...
  auto it = std::remove_if(
      m_heaps.begin(), m_heaps.end(), [](const DescriptorHeap& heap) {
...
void DescriptorPool::CreateHeap(uint32_t numDescriptors,
                                D3D12_DESCRIPTOR_HEAP_FLAGS heapFlags) {
  ...
  THROW_IF_FAILED(
      m_device->CreateDescriptorHeap(&desc, IID_PPV_ARGS(heap.GetAddressOf())));
