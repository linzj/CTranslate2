...
void benchmark_gemm_float_20_1_1_64(Device device) {
  const dim_t batch_size = 20;
  const dim_t m = 1;
  const dim_t k = 64;
  const dim_t n = 1;
  ...
  auto run_gemm = [&](StorageView& a, StorageView& b, StorageView& c,
                      Device d) {
    const dim_t lda = k;
    const dim_t ldb = k;
    const dim_t ldc = n;
    const dim_t stridea = 1 * m * k;
    const dim_t strideb = 1 * n * k;
    const dim_t stridec = 1 * m * n;
    if (d == Device::CPU) {
      primitives<Device::CPU>::gemm_batch_strided<float, float>(
          transpose_a, transpose_b, m, n, k, alpha, a.data<float>(), lda,
          stridea, b.data<float>(), ldb, strideb, beta, c.data<float>(), ldc,
          stridec, batch_size);
    }
#if defined(CT2_WITH_CUDA)
    else if (d == Device::CUDA) {
      primitives<Device::CUDA>::gemm_batch_strided<float, float>(
          transpose_a, transpose_b, m, n, k, alpha, a.data<float>(), lda,
          stridea, b.data<float>(), ldb, strideb, beta, c.data<float>(), ldc,
          stridec, batch_size);
    }
    ...
    else if (d == Device::DirectML) {
      primitives<Device::DirectML>::gemm_batch_strided<float, float>(
          transpose_a, transpose_b, m, n, k, alpha, a.data<float>(), lda,
          stridea, b.data<float>(), ldb, strideb, beta, c.data<float>(), ldc,
          stridec, batch_size);
    }
  ...
  if (!kCheckCorrectness) {
    const Shape a_shape = {batch_size, 1, m, k};
    const Shape b_shape = {batch_size, 1, n, k};
    StorageView a(a_shape, DataType::FLOAT32, device);
    StorageView b(b_shape, DataType::FLOAT32, device);
    StorageView c(DataType::FLOAT32, device);
    c.resize({batch_size, 1, m, n});

    auto fn = [&]() { run_gemm(a, b, c, device); };
    BENCHMARK(fn(), 1000);

  } else {
    const DataType dtype = DataType::FLOAT32;
    const DataType output_dtype = DataType::FLOAT32;

    const Shape a_shape = {batch_size, 1, m, k};
    const Shape b_shape = {batch_size, 1, n, k};
    const Shape c_shape = {batch_size, 1, m, n};

    std::vector<float> a_data = rand_vector(batch_size * 1 * m * k);
    std::vector<float> b_data = rand_vector(batch_size * 1 * n * k);

    StorageView a_device(a_shape, a_data, device);
    StorageView b_device(b_shape, b_data, device);
    StorageView c_device(output_dtype, device);
    c_device.resize(c_shape);

    StorageView a_cpu(a_shape, a_data, Device::CPU);
    StorageView b_cpu(b_shape, b_data, Device::CPU);
    StorageView c_cpu(output_dtype, Device::CPU);
    c_cpu.resize(c_shape);

    run_gemm(a_device, b_device, c_device, device);
    run_gemm(a_cpu, b_cpu, c_cpu, Device::CPU);

    StorageView c_device_cpu_copy(output_dtype, Device::CPU);
    c_device_cpu_copy.copy_from(c_device, true);

    std::ostringstream error_log;
    int total_mismatches = dispatch_compare_views(
        c_device_cpu_copy, c_cpu, "GEMM Float (20,1,1,64) Output", error_log);
    ...
    if (total_mismatches > 0) {
      ...
      throw std::runtime_error(
          ...
          dtype_str_local(dtype) + " (output dtype " +
          dtype_str_local(output_dtype) + ") details:\n" + error_log.str());
    }
  }
}
...
int main(int argc, char* argv[]) {
  ...
  else if (op == "gemm_float_specific")
    benchmark_gemm_float_20_1_1_64(device);
  else if (op == "quantize")
